...
=== Run the Development Mode build server

Now run Quarkus in development mode. *You should only ever have to run this task once* since this enables a hot deployment 
...
`*Update the empty 'src/main/resources/import.sql' file`* so the SQL contents are as follows:
[source,sql]
.import.sql
----
...
Uncomment the 'uberJar' reference at `*line 63*` in the file so it looks like the snippet below:
