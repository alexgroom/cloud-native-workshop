...
**quarkus:dev** runs Quarkus in development mode. *You should only ever have to run this task once* since this enables a hot deployment 
server with background compilation. 

This means that when you modify your Java files and/or your resource files and refresh your browser, the changes will 
...
You will be asked whether you want to connect to the application and it will launch a browser window 
inside your workspace.

If a popup asks to access the debug service on port 5005 then just close this dialog.
...
[NOTE]
====
You don't need to press a save button! Che automatically saves the changes made to the files and the hot
deployment server will update the running code.
====
...
`*Create the 'src/main/resources/import.sql' file`*. Make sure you place it next to the 'applications.properties' file you just edited since there are two *resources* directories very close to each other - you want the higher level one. The SQL contents are as follows:
[source,sql]
.import.sql
----
...
`*No need to execute a build, just refresh your browser and click on 'Test it'*`. You should have the following output:
...
Add the 'uberJar' entry at `*line 63*` in the file.
...
----
<1> uberJar configuration option at the line 63
